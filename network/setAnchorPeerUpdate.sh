...
infoln "Modify the configuration to append the anchor peer"
Modify the configuration to append the anchor peer 
jq '.channel_group.groups.Application.groups.Org2MSP.values += {"AnchorPeers":{"mod_policy": "Admins","value":{"anchor_peers": [{"host": "peer1.org2.example.com","port": 9051}]},"version": "0"}}' config.json > modified_config.json
{ set +x; } 2>/dev/null
